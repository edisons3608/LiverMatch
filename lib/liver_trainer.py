...
class Trainer(object):
    ...
    def inference_one_batch(self, inputs, phase):
        ...
        loss_info['mat_loss'] = float(loss_info['mat_loss'].detach())
        loss_info['vis_loss'] = float(loss_info['vis_loss'].detach())
    ...
    def inference_one_epoch(self, epoch, phase):
        ...
        for key, value in stats_meter.items():
            message += f'{key}: {value.avg:.2f}\t'
            self.writer.add_scalar('epoch/'+f'{phase}/{key}', value.avg, epoch)
        self.logger.write(message + '\n')
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.start_epoch, self.max_epoch)):
            ...
            self.scheduler.step()
            #stats_meter = self.inference_one_epoch(epoch, 'test')
            if stats_meter['loss'].avg < self.best_loss:
                self.best_loss = stats_meter['loss'].avg
                self._snapshot(epoch, 'best_loss')#+str(epoch)
