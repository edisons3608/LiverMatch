...
def get_gt_correspondences(
    ...
    r"""Generate ground-truth superpoint/patch correspondences.
    Each patch is composed of at most k nearest points of the corresponding superpoint.
    A pair of points match if the distance between them is smaller than `self.pos_radius` for fine level.
    A pair of cluster match if they have overlapes.
    Args:
        ref_nodes: torch.Tensor (M, 3)
        src_nodes: torch.Tensor (N, 3)
        ref_knn_points: torch.Tensor (M, K, 3)
        ...
        src_knn_masks (optional): torch.BoolTensor (N, K) (default: None)
    Returns:
        corr_indices_coarse: torch.LongTensor (C, 2)
        corr_indices_fine: torch.LongTensor (C, 2)
        corr_overlaps: torch.Tensor (C,)
    """
    ...
    # compare_pcd(src_nodes.cpu(), ref_nodes.cpu())
    ...
    # corr knn points
    sel_cluster, sel_ref_knn_indices, sel_src_knn_indices = torch.nonzero(point_overlap_mat, as_tuple=True)
    corr_indices_knn = torch.stack([sel_cluster, sel_src_knn_indices, sel_ref_knn_indices], dim=1)

    return corr_indices, corr_indices_knn, corr_overlaps
